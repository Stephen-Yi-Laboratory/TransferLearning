...
iData <- read.csv('./data/drug_cell_line.csv')
...
findDrugCombinations <- function(iData, tumorComposition, plotTitle = NA, lSize = 2.2, nLabels = 5, outFile){
  ...
  dPCH <- ifelse(((df$sActivity > 0) & (df$P < 0.02)),8,16) # FOR P_VALUE
...
#P6 <- P6 +
 # stat_compare_means(comparisons = list(c('TNBC', 'HER2+'),c('TNBC','ER+')),
  #                   method = 'wilcox.test',
   #                  label = 'p.format',  # Changed from 'p.signif'
    #                 label.y = c(0.9, 0.95),
...
P6 <- P6 +
  stat_compare_means(comparisons = list(c('TNBC', 'HER2+'),c('TNBC','ER+')),
                     ...
                     bracket.size = 0.2, 
                     tip.length = 0.005,
                     family = "sans")  # Specify font family explicitly
